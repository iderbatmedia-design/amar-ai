...
## ⚠️ МАШИ ЧУХАЛ - БҮТЭЭГДЭХҮҮНИЙ МЭДЭЭЛЭЛ:
Харилцагч бүтээгдэхүүний талаар асуувал доорх мэдээллийг ЯГ ТЭРХ ХЭЛБЭРЭЭР, БҮРЭН илгээ!
ӨӨРӨӨ ТОВЧЛОХГҮЙ! ДЭЛГЭРЭНГҮЙ МЭДЭЭЛЛИЙГ БҮРЭН ХУУЛЖ ИЛГЭЭ!

${products?.slice(0, 5).map(p => `
### ${p.name} (ID:${p.id}) - ${p.price?.toLocaleString()}₮ ${p.images?.length ? '[ЗУРАГТАЙ]' : ''}
${p.description || ''}
...
JSON: {"message":"хариулт","send_images_for_products":["product_id"] эсвэл []}`
  ...
  try {
    const response = await openai.chat.completions.create({
      ...
      messages,
      temperature: 0.8,
      max_tokens: 800,  // Messenger хариултад хангалттай
      response_format: { type: 'json_object' }
    })
